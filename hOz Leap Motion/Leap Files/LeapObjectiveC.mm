...
{
    const LeapVector *n = [axis normalized];
    const float s = std::sin(angleRadians);
    const float c = std::cos(angleRadians);
    const float C = (1-c);
    
    _xBasis = [[LeapVector alloc] initWithX:(n.x * n.x * C + c) y:(n.x * n.y *C - n.z * s) z:(n.x * n.z * C + n.y * s)];
    _yBasis = [[LeapVector alloc] initWithX:(n.y * n.x * C + n.z * s) y:(n.y * n.y * C + c) z:(n.y * n.z * C - n.x * s)];
    _zBasis = [[LeapVector alloc] initWithX:(n.z * n.x * C - n.y * s) y:(n.z * n.y * C + n.x * s) z:(n.z * n.z * C + c)];
}
